...
if (isset($_GET['no'])) {
    ...
    $course_no = $_GET['no'];
    //檢查REFERER避免JSON被人盜用
    if ((!isset($_SERVER['HTTP_REFERER']) || strpos($_SERVER['HTTP_REFERER'], $_SERVER['SCRIPT_NAME']) === false) && !isset($DEBUG)) {
        $output = '抱歉喔，請從正常管道使用本服務';
    }
    //如果課程代碼的長度不等於9就印出error並離開
    else if (strlen($course_no) == 9) {
        //課程查詢的網址
        ...
        $options = array(
            ...
            CURLOPT_USERAGENT => "Mozilla/5.0 (Windows NT 6.1) Chrome",
            CURLOPT_POST => true,
...
//        print_r($result);  //DEBUG
        ...
        //取得課程名稱
        if (preg_match('/課程名稱.+<font color="#4A3C8C">(.+)<\/font>/', $result, $str)) {
            $course_name = trim($str[1]);
            //取得目前選課人數
            if (preg_match('/合班選課人數：(\d+)　/', $result, $str)) {
                $number_now = $str[1];
            } else if (preg_match('/目前選課人數.+<font color="#4A3C8C">(\d+)</', $result, $str)) {
                $number_now = $str[1];
            }
            //取得加退選課人數上限
            if (preg_match('/加退選課人數上限.+<font color="#4A3C8C">(\d+)</', $result, $str)) {
                $number_max = $str[1];
